...
pub fn render(workspace: &mut Workspace, frame: &mut Frame) {
    ...
    // 保存编辑器区域（用于鼠标事件）
    workspace.editor.view.set_editor_area(editor_area);
    
    // 在渲染前更新视口状态（状态更新集中在此处，包括水平滚动）
    workspace.editor.update_viewport(viewport_height, viewport_width);
    
    // 只渲染可见行（零分配版本：使用 Vec<Line> 而非 String）
    ...
    // 计算光标位置（相对于视口）
    let cursor = workspace.editor.cursor();
    let cursor_x = editor_area.x + get_cursor_display_x(&mut workspace.editor);
    let cursor_y = editor_area.y + (cursor.0 - workspace.editor.viewport_offset()) as u16;
